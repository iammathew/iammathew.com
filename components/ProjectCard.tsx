...
const ProjectCard = ({ title, description, imgSrc, href }) => (
  <div className="md p-4 md:w-full">
    ...
    >
      <div className="relative mr-0 min-h-[300px] flex-1 basis-5/12 md:mr-4  md:min-h-[220px]">
        {imgSrc &&
          (href ? (
            <Link href={href} aria-label={`Link to ${title}`}>
              <Image
                alt={title}
                src={imgSrc}
                className="rounded-xl object-center dark:bg-white"
                layout="fill"
                objectFit="cover"
              />
            </Link>
            ...
            <Image
              alt={title}
              src={imgSrc}
              className="h-20 rounded-xl object-center dark:bg-white"
              layout="fill"
              objectFit="cover"
            />
          ))}
